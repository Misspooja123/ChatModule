...
export class ChatService {
  constructor(
    ...
    @InjectModel(Status.name, 'chatdb') private readonly statusModel: Model<SyncInfo>,
  ) { }
    ...
    inbox = await Promise.all(inbox.map(async (value: any) => {
      ...
      return {
        ...
        chatMessages: await Promise.all(value?.chatMessages.map(async (chatMesg: any) => {
          ...
          let userName= ( String(value.sender._id) === String(chatMesg.from)) ? value?.sender?.data?.first_name + " " + value?.sender?.data?.last_name : value?.reciever?.data?.first_name + " " + value?.reciever?.data?.last_name;          
          chatMesg.userName = userName
          chatMesg.messageType = chatMesg.messageData.messageType          
  ...
  async updateStatus(filter: any, update: any): Promise<any> {
    return await this.statusModel.findOneAndUpdate(filter, update, { new: true });
  }


  async createStatus(data: object): Promise<any> {
    const message = new this.statusModel(data);
    await message.save();
    return message
  }

  async isExist(filter: any): Promise<any> {
    return await this.statusModel.findOne(filter);
  }

  async updateManyChats(filter: any, update: any): Promise<any> {
    console.log("filter", filter);
    let result = await this.chatModel.updateMany(filter, update);
  ...
  async getConversation(filter: any): Promise<any> {
    return await this.inboxModel.find(filter).exec();
  }
