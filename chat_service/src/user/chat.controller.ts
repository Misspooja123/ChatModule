...
export class ChatController {
  ...
  async chatMessage(@Payload() message: any, req: Request, res: Response) {
    try {
      ...
      let findConversation = await this.chatService.getConversation({
        from: new mongoose.Types.ObjectId(message.from),
        to: new mongoose.Types.ObjectId(message.to),
        convType: message.convType
      });

      const isBlocked = findConversation[0]?.isBlocked;
      ...
      chatMessage.convType = message.convType
  ...
  @MessagePattern({ cmd: 'isOnline' })
  async onlineSync(@Payload() message: any, req: Request, res: Response) {
    try {
      let exist = await this.chatService.isExist({
        userId: new mongoose.Types.ObjectId(message.userId)
      });
      
      if (exist) {
        let updateStatus = await this.chatService.updateStatus({
          userId: new mongoose.Types.ObjectId(message.userId)
         }, {
           // $push: { deliver: { [message.to] : new Date() } },
           $set: {isOnline: 1,lastSeen: new Date()} // Additional fields to update
         });
          return updateStatus;
      } else {
        let chatMessage = await this.chatService.createStatus({
          userId: new mongoose.Types.ObjectId(message.userId),
          isOnline: 1,
          lastSeen: new Date()
        });
  ...
  @MessagePattern({ cmd: 'isOffline' })
  async isOfflineSync(@Payload() message: any, req: Request, res: Response) {
    try {
      ...
      if (exist) {
        let updateStatus = await this.chatService.updateStatus({
         userId: new mongoose.Types.ObjectId(message.userId)
          ...
          // $push: { deliver: { [message.to] : new Date() } },
          $set: {isOnline: 0,lastSeen: new Date()} // Additional fields to update
        });
         return updateStatus;
      }
  ...
  async deleteMessage(@Payload() message: any, req: Request, res: Response) {
    try {
      let convType = message.convType // 1 for user, 2 for group, 3 for broadcast
      let deleteType = (message.deleteType?.toString() ?? "1")
      let userId = message.userId?.toString() ?? "0"
      let mId = message.mId ?? []

      /**
       *   type : 1 - delete for me, 2 - delet
       * 
       * 
       * e for both
       *   userId(senderID) : xx
  ...
  async editMessage(@Payload() message: any, req: Request, res: Response) {
    try {
      ...
      console.log("here",oldChatMessage.messageData.text);
      ...
      console.log("messageData1",messageData);
  ...
  async blockUser(@Payload() message: any, req: Request, res: Response) {
    try {
      ...
      return await this.chatService.updateInbox({
        ...
        $set: { isBlocked: message.isBlock, clearedAt: new Date(), lastMessageId: null }
      });
    } catch (error) {
      logger.info("chat block_user");
      logger.info(error);
      throw error;
    }
  }
  ...
  async getMessages(@Payload() message: any, req: Request, res: Response) {
    try {
      let { lastId, from, to, convType } = message
      const convCount: any = await this.chatService.getInboxCount({
      ...
      if (convCount) {
        ...
        messages = await Promise.all(chatMessages.map(async (value: any) => {
          value.messageType = value.messageData.messageType
