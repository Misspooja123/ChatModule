...
export class UserController {
  ...
  @MessagePattern({ cmd: 'media_upload' })
  async imageUpload(@Payload() message: any, req: Request, res: Response) {

    try {
      console.log("meesage",message);
      const filenames = message.files.map((file:any) => file.filename);
      const images = filenames.map((filename:any) => ({
        image_name: filename,
        path: `localhost:9001/public/${filename}`
      }));

      return images;
    } catch (error) {
      logger.info("user media_upload");
      logger.info(error);
      throw error;
    }
  }
