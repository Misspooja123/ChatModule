import { Controller, Post, Body, Inject, Req, Res, Get, Headers, Patch, HttpException, HttpStatus, UploadedFile, UseInterceptors, UnauthorizedException, Delete, Ip, Param, UploadedFiles } from '@nestjs/common';
...
export class UserController {
  ...
  async mediaUpload(@UploadedFiles() files, @Headers() headers: any) {
    try {
      return this.userClient.send({ cmd: 'media_upload' }, { files, headers })
        .pipe(
          catchError(errors => throwError(() => new HttpException({
            message: errors.message ?? 'Input data validation failed',
            errors: errors.errors ?? null
          }, errors.statusCode ?? HttpStatus.UNPROCESSABLE_ENTITY)))
        );
    } catch (error) {
      logger.info("api-gateway media-upload");
