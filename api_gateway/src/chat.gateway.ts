...
export class ChatGateway implements OnGatewayConnection {
  ...
  handleConnection(client: Socket) {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      console.log("Invalid user ID detected. Closing connection...");
    ...
    this.chatClient.send({ cmd: 'isOnline' }, { userId }).subscribe({
      next: (result) => {
        console.log(result);
        client.emit("confirmConnect", result)
      },
      error: (error) => {
    ...
    this.chatClient.send({ cmd: 'offline_sync' }, { userId }).subscribe({
      next: (result) => {
        console.log(result);
    ...
    // client.emit("confirmConnect", { connection: true })

    // let conversationsJoin = await Conversations.findAll({
    //     where: {
    ...
    // conversations.map(value => value.chatId).forEach(chatId => {
    //     client.leave(chatId)
    //     io.to(chatId).emit("online", {
    //         "from": Number(userId),
    //         "status": 1
    //     })
    ...
    // client.join(conversationsJoin.map(value => value.chatId))
    ...
    //           // client.join(result.senderId);
    //           // const sId = AppConstants.userMap[to?.toString()]?.socket_id;
    //           // if (sId) {
    //           //   io.sockets.sockets.get(sId)?.join(result.senderId);
    //           // }
    //           // client.to(result?.senderId.toString())?.emit("chatMessage", result)
    //           ack(result);
    //         },
    //         error: (error) => {
          ...
          .subscribe({
            next: (result) => {
              ...
              if (result[0]?.isBlocked) {
                return client.broadcast.emit(`chatMessage`, result);
              }
          ...
          next: (result) => {
            ...
            if (sIds && sIds.length) {
              ...
              this.chatClient.send({ cmd: 'offline_store' }, {
                ...
                data: emitData, //{ ...emitData, "status": MessageStatus.DELIVERED.valueOf() },
