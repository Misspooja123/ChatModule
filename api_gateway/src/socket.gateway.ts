...
export class SocketGateway implements OnGatewayConnection {
  ...
  constructor(
          ...
          let output = result.adminData.map((data:any)=>{
            ...
            if (this.connectedUsers.get(client.id)?.length) {
              ...
              // console.log('role id',role_)
                ...
                // console.log({"this.connectedUsers": this.connectedUsers.values()});
                // for (const [key, socketId] of this.connectedUsers) {
                //   console.log({socketId, "client.id":client.id});
                //   client.to(client.id).emit("roleHasPermissionUpdateEvent", {
                //     role: role_,
                //     roleId: roleId
                // });
                // }
                // Object.values(this.connectedUsers).forEach((socketId: string) => {
                // client.broadcast.emit("roleHasPermissionUpdateEvent", {
                //   role_, roleId
                // });
                // console.log({socketId, "client.id":client.id});
    ...
    // client.on('update_data', async (data:any) => {
    //   console.log("role by raja out", {data});

    //   this.adminClient.send({ cmd: 'sub_admin_list' }, '65f2a3c35139bc678165dd32')
    //     .subscribe({
    //       next: (result) => {   
    //         console.log("role by raja in", result);
    //         role_ = result.role_Data;
    //         const roleId = data.role_id;

    //         if (this.connectedUsers.get(client.id)?.length) {
    //           let data = { role_, roleId };
    //           client.broadcast.emit("roleHasPermissionUpdateEvent-" + roleId, data);
    //         }
    //       },
    //       error: (error) => {
    ...
    // console.log("connectedUsers", Array.from(this.connectedUsers.keys()));
    ...
    client.on('maintenace_mode', async (data, ack) => {
      ...
      // Array.from(this.connectedUsers.keys()).map((el)=>{
      //   client.broadcast.emit(el + '_chat', data);
      // })
    })
    ...
    client.on('chat_message', async (data, ack) => {
      ...
      if (isUserIdPresent) {
        ...
        // client.broadcast.emit('user_support', data);
        // client.broadcast.emit(data["to"]+ '_chat', data);
        // console.log("data[from]]",this.connectedUsers.get(client.id));
    ...
    //              delivered offline message event
    // client.on('message_deliver_offline', async(data,ack) => {
    //   console.log(`Received from User ${client.id}: ${this.connectedUsers.get(client.id)}`);
    //   let userId = this.connectedUsers.get(client.id)
    //   let message = {
    //     data: data.data ?? null,
    //     to: data.to ?? null,
